...
interface EnglishGamePaymentButtonProps {
  onSuccess?: () => void;
}

export function EnglishGamePaymentButton({ onSuccess }: EnglishGamePaymentButtonProps) {
  const piAuth = usePiAuth();
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [successMessage, setSuccessMessage] = useState<string | null>(null);

  // Find the product from the products array using the specific product ID
  const product = piAuth?.products?.find(
    (p) => p.id === PRODUCT_CONFIG.PRODUCT_69eeb67e8405571765884618
  );
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      if (result.ok) {
        setSuccessMessage(
          `✓ Chúc mừng! Bạn đã nâng cấp lên bản Premium`
        );
        
        // Log successful purchase details
        console.log('[English Game Premium Payment] Success', {
      ...
      console.error('[English Game Premium Payment] Error:', err);
    } finally {
      setIsLoading(false);
    }
  };
    ...
    return (
      <div className="bg-gradient-to-br from-green-50 to-green-100 rounded-3xl p-4 shadow-sm border border-green-200">
        <div className="flex items-center gap-2 text-green-700 font-bold">
          ...
          <span>Bạn đã nâng cấp lên bản Premium</span>
        </div>
      </div>
    );
  }
